...
var addressTemplate = require('./addressPanelTemplate.string');
var popPanel = {
  cityInfo: {
                "北京": ["北京"],
                "上海": ["上海"],
  ...
  show : function(params){
    this.$panelContainer = $('.pop-panel-con');
    this.data = params;
  ...
  loadPanel : function(){
    var popPanelHtml = _ct.renderHtml(addressTemplate,this.data.addressData);
    this.$panelContainer.html(popPanelHtml);
    this.loadProvince();
  },
    ...
    //关闭按钮
    $(document).on('click', '.popPanel .header .close',function(){
      _this.hide();
    });
    ...
    $(document).on('click', '.btn.saveAddress',function(){
      var shippingInfo = {};
      var updateTag = _this.data.updateTag;
      shippingInfo.receiverName = $.trim(_this.$panelContainer.find('#receive-name').val());
      shippingInfo.receiverPhone = $.trim(_this.$panelContainer.find('#phone').val());
      shippingInfo.receiverProvince = $.trim(_this.$panelContainer.find('#province').val());
      shippingInfo.receiverCity = $.trim(_this.$panelContainer.find('#city').val());
      shippingInfo.receiverAddress = $.trim(_this.$panelContainer.find('#address').val());
      shippingInfo.receiverZip = $.trim(_this.$panelContainer.find('#postcode').val());

      //数据监测
      var valideateRes = _this.shippingInfoValidate(shippingInfo);
      if(valideateRes.success){
        //提交
        if(updateTag){
          //更新请求
          shippingInfo.id = _this.data.addressData.id;
          _shippingAddress.updateAddress(shippingInfo,
            function(res){
              _this.hide();
              _this.data.onSuccess();
            },
            function(errMsg){
          ...
          //添加新地址请求
          _shippingAddress.addAddress(shippingInfo,
            function(res){
              _this.hide();
              _this.data.onSuccess();
            },
        ...
        _ct.errorTips(valideateRes.msg);
      }
  ...
  loadProvince : function(){
    ...
    if(_this.data.updateTag && _this.data.addressData.receiverProvince){
      //回填信息
      $('#province').val(_this.data.addressData.receiverProvince);
      _this.loadCity(_this.data.addressData.receiverProvince);
      $('#city').val(_this.data.addressData.receiverCity);
    }
  ...
  loadCity : function(province){
    var _this = this;
    var cities = this.cityInfo[province] || [];
    var optionHtml = '<option value="">请选择</option>';
    for(var i = 0, length = cities.length; i < length; i++){
      optionHtml += '<option value="' + cities[i] + '">' + cities[i] + '</option>';
    }
    $('#city').html(optionHtml);
  },

  //数据检查
  shippingInfoValidate : function(shippingInfo){
    var result = {
      success : false,
      msg : ''
    };
    ...
    if(!_ct.validate(shippingInfo.receiverPhone,'phone')){
      result.msg = '手机号格式不正确';
      return result;
    }

    if(!shippingInfo.receiverProvince){
      result.msg = '请选择省份';
      return result;
    }

    if(!shippingInfo.receiverCity){
      result.msg = '请选择城市';
      return result;
    }

    if(!shippingInfo.receiverAddress){
      result.msg = '请输入详细地址';
      return result;
    }
    result.success = true;
